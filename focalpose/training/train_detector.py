...
def train_detector(args):
    ...
    def make_datasets(dataset_names, two_class):
        datasets = []
        all_labels = set()
        for (ds_name, n_repeat) in dataset_names:
            ...
            if two_class:
                all_labels = all_labels.union(set(['object']))
            else:
    ...
    scene_ds_train, train_labels = make_datasets(args.train_ds_names, args.two_class)
    scene_ds_val, _ = make_datasets(args.val_ds_names, args.two_class)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if get_rank() == 0:
            log(config=args, model=model, epoch=epoch,
                log_dict=log_dict, test_dict=None)
        dist.barrier()
